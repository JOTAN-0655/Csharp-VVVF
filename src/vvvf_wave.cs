...
namespace VVVF_Generator_Porting
{
	public class vvvf_wave
	{
		...
		public static Wave_Values calculate_three_level(Pulse_Mode pulse_mode, double expect_saw_angle_freq, double initial_phase, double amplitude)
		{
			...
			double pwm_value = 0.5 * (get_pwm_value(sin_value, .5 * saw_value + 0.5) + get_pwm_value(sin_value, 0.5 * saw_value - 0.5));
		...
		public static Wave_Values calculate_jre_209_mitsubishi_gto(bool brake, bool mascon_on, bool free_run, double initial_phase, double wave_stat)
		{
			double amplitude = 0;
			...
			if (brake)
            {
				amplitude = get_Amplitude(wave_stat, 68);
				if (59 <= wave_stat)
				{
					pulse_Mode = Pulse_Mode.P_1;
					amplitude = get_Amplitude(wave_stat, 72) + get_Amplitude(wave_stat-59, 15);
				}
				else if (49 <= wave_stat || (free_run && sin_angle_freq > 49 * M_2PI))
					pulse_Mode = Pulse_Mode.P_3;
				else if (40 <= wave_stat || (free_run && sin_angle_freq > 40 * M_2PI))
					pulse_Mode = Pulse_Mode.P_9;
				else if (19 <= wave_stat || (free_run && sin_angle_freq > 19 * M_2PI))
            ...
            {
				amplitude = get_Amplitude(wave_stat, 62);
				if (53 <= wave_stat)
                {
					pulse_Mode = Pulse_Mode.P_1;
					amplitude = get_Amplitude(wave_stat, 66) + get_Amplitude(wave_stat - 53, 15);
				}
				else if (46 <= wave_stat || (free_run && sin_angle_freq > 46 * M_2PI))
					pulse_Mode = Pulse_Mode.P_3;
				else if (30 <= wave_stat || (free_run && sin_angle_freq > 30 * M_2PI))
					pulse_Mode = Pulse_Mode.P_9;
				else if (19 <= wave_stat || (free_run && sin_angle_freq > 19 * M_2PI))
					pulse_Mode = Pulse_Mode.P_21;
				else if (9 <= wave_stat || (free_run && sin_angle_freq > 9 * M_2PI))
					pulse_Mode = Pulse_Mode.P_33;
				else if (2 <= wave_stat || (free_run && sin_angle_freq > 2 * M_2PI))
			...
			return calculate_three_level(pulse_Mode, expect_saw_angle_freq, initial_phase, amplitude);
		}
		...
		public static Wave_Values calculate_silent(bool brake, bool mascon_on, bool free_run, double initial_phase, double wave_stat)
		{
			...
			{
				pulse_Mode = Pulse_Mode.Not_In_Sync;
				expect_saw_angle_freq = get_random_freq(550, 20) * M_2PI;
			}
		...
		public static Wave_Values calculate_225_5100_mitsubishi(bool brake, bool mascon_on, bool free_run, double initial_phase, double wave_stat)
		{
			...
			if (!brake)
			{
				...
				else
				{
					...
					pulse_Mode = Pulse_Mode.Not_In_Sync;
					expect_saw_angle_freq = get_random_freq(1050, 20) * M_2PI;
				}
			}
			else
			{
		...
		public static Wave_Values calculate_keio_8000_gto(bool brake, bool mascon_on, bool free_run, double initial_phase, double wave_stat)
		{
			...
			if (brake)
			{
				...
				if (68.2 <= wave_stat) pulse_Mode = Pulse_Mode.P_1;
				...
				else if (54.5 <= wave_stat || (free_run && sin_angle_freq > 54.5 * M_2PI)) pulse_Mode = Pulse_Mode.P_3;
				else if (41.2 <= wave_stat || (free_run && sin_angle_freq > 41.2 * M_2PI)) pulse_Mode = Pulse_Mode.P_7;
				else if (32.3 <= wave_stat || (free_run && sin_angle_freq > 32.3 * M_2PI)) pulse_Mode = Pulse_Mode.P_11;
				else if (21.0 <= wave_stat || (free_run && sin_angle_freq > 21.0 * M_2PI)) pulse_Mode = Pulse_Mode.P_15;
				else if (7.8 <= wave_stat || (free_run && sin_angle_freq > 7.8 * M_2PI)) pulse_Mode = Pulse_Mode.P_21;
				else
				{
					Wave_Values wv = new Wave_Values();
					wv.sin_value = 0;
			...
			else
			{
				...
				if (50 <= wave_stat) pulse_Mode = Pulse_Mode.P_1;
				...
				else if (41.2 <= wave_stat || (free_run && sin_angle_freq > 41.2 * M_2PI)) pulse_Mode = Pulse_Mode.P_3;
				else if (32.4 <= wave_stat || (free_run && sin_angle_freq > 38.4 * M_2PI)) pulse_Mode = Pulse_Mode.P_7;
				else if (29.5 <= wave_stat || (free_run && sin_angle_freq > 29.5 * M_2PI)) pulse_Mode = Pulse_Mode.P_11;
				else if (25.8 <= wave_stat || (free_run && sin_angle_freq > 25.8 * M_2PI)) pulse_Mode = Pulse_Mode.P_15;
				else
				{
					pulse_Mode = Pulse_Mode.Not_In_Sync;
					expect_saw_angle_freq = M_2PI * 400;
